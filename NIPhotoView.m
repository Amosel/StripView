...
@interface NIPhotoView()
@property (nonatomic, readwrite, assign) NIPhotoViewPhotoSize photoSize;
...
@synthesize photoStripViewDelegate = _photoStripViewDelegate;
...
- (void)didDoubleTap:(UITapGestureRecognizer *)tapGesture {
  ...
  if ([self.photoStripViewDelegate respondsToSelector:
       @selector(photoScrollViewDidDoubleTapToZoom:didZoomIn:)]) {
    [self.photoStripViewDelegate photoScrollViewDidDoubleTapToZoom:self didZoomIn:didZoomIn];
  }
}
...
#pragma mark -
#pragma mark NIStripViewItem



- (void)prepareForReuse {
  _imageView.image = nil;
  self.photoSize = NIPhotoViewPhotoSizeUnknown;
...
- (void)minAndMaxScaleForDimensions: (CGSize)dimensions
                         boundsSize: (CGSize)boundsSize
                          photoSize: (NIPhotoViewPhotoSize)photoSize
  ...
  CGFloat maxScale = ((NIPhotoViewPhotoSizeUnknown == photoSize)
                      ? 1
                      : (1.0f / NIScreenScale()));

  if (NIPhotoViewPhotoSizeThumbnail != photoSize) {
...
- (void)setMaxMinZoomScalesForCurrentBounds {
  ...
  if ((NIPhotoViewPhotoSizeThumbnail == self.photoSize)
